...
## Module
`tabs/reference/reference.js` — extracted from dashboard.html 2026-05-09 via CapStudio modular loader.
...
## Common debug
- Console: look for `[shell] rendered tab 'reference' from module …`
- Module render error: `[shell] module render 'reference' threw: …`
- Missing helper: add `window.X = X` exposure in source HTML

## Capability registry
`data/capability_registry.json` → `tabs.reference`
