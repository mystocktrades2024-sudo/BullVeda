...
## Module
`tabs/accuracy/accuracy.js` — extracted from dashboard.html 2026-05-09 via CapStudio modular loader.
...
## Common debug
- Console: look for `[shell] rendered tab 'accuracy' from module …`
- Module render error: `[shell] module render 'accuracy' threw: …`
- Missing helper: add `window.X = X` exposure in source HTML

## Capability registry
`data/capability_registry.json` → `tabs.accuracy`
