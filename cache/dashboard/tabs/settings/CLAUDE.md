...
## Module
`tabs/settings/settings.js` — extracted from dashboard.html 2026-05-09 via CapStudio modular loader.
...
## Common debug
- Console: look for `[shell] rendered tab 'settings' from module …`
- Module render error: `[shell] module render 'settings' threw: …`
- Missing helper: add `window.X = X` exposure in source HTML

## Capability registry
`data/capability_registry.json` → `tabs.settings`
