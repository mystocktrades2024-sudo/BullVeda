...
## Module
`subtabs/options/options.js` — extracted from elite-detail.html 2026-05-09 via CapStudio modular loader.
...
## Common debug
- Console: look for `[detail-shell] rendered tab 'options' from module …`
- Module render error: `[detail-shell] module render 'options' threw: …`
- Missing helper: add `window.X = X` exposure in source HTML

## Capability registry
`data/capability_registry.json` → `sub_tabs.options`
