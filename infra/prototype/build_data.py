...
# ── Per-mode decision-rule schedule (2026-05-12) ────────────────────────────
# Fixes the Principle-18 violation where the detail-view mode toggle was
# cosmetic: SWING-mode trade plan, R:R floor, ER window, entry quality
# threshold, and Kelly multiplier were used regardless of which mode label
# was shown. This block defines mode-specific decision rules sourced from
# fund-practice norms + existing config. Used by _compute_modes_block().
#
# Source citations for the parameter set (do not change without _validations
# entry per CLAUDE.md Principle 7):
#   - swing stop_atr_mult 1.25 → config.scoring.stop_atr_multiple (legacy)
#   - position stop_atr_mult 1.75 → wider for 10-90d hold; matches the
#     score_medium_term position-bias check at analysis.py (uses 2×ATR floor)
#   - invest stop = -15% drawdown → standard CFP suitability bound;
#     "thesis break" alternative is structural, hard to encode
#   - R:R floors 3.0 / 2.5 / 2.0 → hedge-fund convention by hold horizon
#   - Kelly mults 1.0 / 1.3 / 1.5 → matches _POSITION_MODE_W RS-tilt + invest
#     fundamentals dominance (longer hold = higher conviction sizing)
#   - ER windows: swing avoids the binary (≥14d clear), position welcomes it
#     (catalyst window), invest is horizon-agnostic on quarterly prints
_MODE_RULES = {
    "swing": {
        "stop_atr_mult":    1.25,
        "t1_atr_mult":      1.5,
        "t2_atr_mult":      2.5,
        "rr_floor":         3.0,
        "er_window_days":   14,        # ER must be ≥14d clear
        "er_rule":          "≥14d clear",
        "entry_quality":    ["FRESH", "PULLBACK"],
        "kelly_mult":       1.0,
        "hold_days":        "2-21",
        "weight_tuple":     (35, 20, 20, 15, 10),   # original 5-pillar
        "rules_note":       "1.25×ATR stop · 3:1 R:R · ER≥14d clear · FRESH/PULLBACK only",
    },
    "position": {
        "stop_atr_mult":    1.75,
        "t1_atr_mult":      3.0,
        "t2_atr_mult":      5.0,
        "rr_floor":         2.5,
        "er_window_days":  -30,        # ER within 30d welcome (catalyst)
        "er_rule":          "within 30d welcome (catalyst window)",
        "entry_quality":    ["FRESH", "PULLBACK", "VALID"],
        "kelly_mult":       1.3,
        "hold_days":        "10-90",
        "weight_tuple":     _POSITION_MODE_W,
        "rules_note":       "1.75×ATR stop · 2.5:1 R:R · ER within 30d welcome · FRESH/PULLBACK/VALID",
    },
    "invest": {
        "stop_pct_drawdown": 0.15,      # -15% from entry (CFP suitability bound)
        "t1_from":          "analyst_pt",   # primary: analyst target mean
        "t1_fallback_pct":  0.30,           # +30% if no analyst PT
        "t2_multiple":      1.5,            # T2 = 1.5 × T1 (long compound)
        "rr_floor":         2.0,
        "er_window_days":   None,           # irrelevant on multi-quarter horizon
        "er_rule":          "irrelevant (multi-quarter hold)",
        "entry_quality":    ["FRESH", "PULLBACK", "VALID", "EXTENDED"],
        "kelly_mult":       1.5,
        "hold_days":        "1-5yr",
        "weight_tuple":     _INVEST_MODE_W,
        "rules_note":       "15% drawdown stop · 2:1 R:R · ER agnostic · compound on quality not structure",
    },
}


def _compute_modes_block(r: dict, base_plan: dict) -> dict:
    """Build per-mode (swing/position/invest) decision payloads for the UI.

    Each mode dict contains: score, verdict, stop, t1, t2, entry_lo, entry_hi,
    rr_ratio, rr_floor, er_window_days, er_rule, entry_quality_req, kelly_mult,
    hold_days_range, rules_note.

    Drives the mode toggle on elite-detail.html — clicking SWING/POSITION/INVEST
    rebinds the Plan Ladder + Pre-Flight Checklist sections to the matching block.

    See _MODE_RULES above for parameter sourcing.
    """
    techs        = r.get("technicals") or {}
    indicators   = techs.get("indicators") or {}
    atr          = float(indicators.get("atr") or 0) or 0.0
    price        = float(r.get("price") or base_plan.get("entry_low") or 0) or 0.0
    analyst_pt   = float(((r.get("analyst") or {}).get("target_mean")) or 0) or 0.0
    swing_stop   = base_plan.get("stop")
    swing_t1     = base_plan.get("target1")
    swing_t2     = base_plan.get("target2")
    entry_lo     = base_plan.get("entry_low")  or base_plan.get("primary_zone_low")
    entry_hi     = base_plan.get("entry_high") or base_plan.get("primary_zone_high")

    # Pull mode-specific scores/verdicts from the existing helper
    mv = _compute_mode_verdicts(r)
    swing_score   = r.get("score") or 0
    swing_verdict = (r.get("decision") or {}).get("verdict") or r.get("verdict") or "WATCH"
    swing_conv    = (r.get("conviction") or {}).get("label") or r.get("conviction_tier") or ""
    ...
    def _build(mode: str) -> dict:
        ...
        if mode == "swing":
            ...
            t1   = swing_t1
            t2   = swing_t2
            score, verdict = swing_score, swing_verdict
        elif mode == "position":
            stop = round(price - rules["stop_atr_mult"] * atr, 2) if (price > 0 and atr > 0) else swing_stop
            t1   = round(price + rules["t1_atr_mult"]   * atr, 2) if (price > 0 and atr > 0) else swing_t1
            t2   = round(price + rules["t2_atr_mult"]   * atr, 2) if (price > 0 and atr > 0) else swing_t2
            score, verdict = mv.get("medium_term_score"), mv.get("medium_term_verdict")
        else:  # invest
            stop = round(price * (1 - rules["stop_pct_drawdown"]), 2) if price > 0 else swing_stop
            t1   = round(analyst_pt, 2) if analyst_pt > 0 else (
                   round(price * (1 + rules["t1_fallback_pct"]), 2) if price > 0 else swing_t1)
            t2   = round(t1 * rules["t2_multiple"], 2) if t1 else swing_t2
            score, verdict = mv.get("long_term_score"), mv.get("long_term_verdict")

        # R:R from entry-mid (or price fallback)
        em = ((entry_lo or 0) + (entry_hi or 0)) / 2 if (entry_lo and entry_hi) else price
        try:
            rr = round(((t1 or 0) - em) / max(em - (stop or 0), 0.01), 2) if (em and stop and t1) else 0
        except Exception:
            rr = 0
        ...
        return {
            ...
            "score":             score,
            "verdict":           verdict,
            "conviction":        swing_conv if mode == "swing" else "—",  # only swing has Wilson-validated conviction
            "stop":              stop,
            "t1":                t1,
            "t2":                t2,
            "entry_lo":          entry_lo,
            "entry_hi":          entry_hi,
            "rr_ratio":          rr,
            "rr_floor":          rules["rr_floor"],
            "rr_pass":           bool(rr >= rules["rr_floor"]) if rr else False,
            "er_window_days":    rules["er_window_days"],
            "er_rule":           rules["er_rule"],
            "entry_quality_req": rules["entry_quality"],
            "kelly_mult":        rules["kelly_mult"],
            "hold_days_range":   rules["hold_days"],
            "weight_tuple":      rules["weight_tuple"],
            "rules_note":        rules["rules_note"],
        }

    return {
        "swing":    _build("swing"),
        "position": _build("position"),
        "invest":   _build("invest"),
        "_baseline": "swing",   # the flat T.* fields reflect this mode
        "_audit": (
            "Mode rules sourced from _MODE_RULES in build_data.py. Swing uses legacy "
            "stop_atr_multiple=1.25; position widens to 1.75×ATR; invest uses -15% drawdown "
            "or analyst PT. Per-mode pillar weights match _POSITION_MODE_W / _INVEST_MODE_W. "
            "ER windows differ: swing avoids binary, position welcomes catalyst, invest agnostic. "
            "NOT mode-aware (still swing-baseline): MC simulations, Wilson backtests, "
            "lens votes, 3-lens technical readings. See open-items MODE-AWARE-DETAIL."
        ),
    }
...
def rich_row(r: dict, b: dict = None) -> dict:
    ...
    base.update({
        ...
        **_compute_mode_verdicts(r),
        # 2026-05-12 — Mode-aware decision payloads (swing/position/invest)
        # for the detail-view mode toggle. Before this, the toggle was
        # cosmetic — swing-mode stop/t1/t2/R:R/ER-window/entry-quality were
        # used regardless of which mode label was shown. T.modes now carries
        # three parallel parameter sets so JS can rebind the Plan Ladder +
        # Pre-Flight Checklist sections per mode. See _compute_modes_block().
        "modes": _compute_modes_block(r, r.get("trade_plan") or {}),
