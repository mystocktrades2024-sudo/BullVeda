  ...
  return (
    <span className={`wilson ${low ? "wilson--low" : ""}`}>
      <span className="wilson-block">
        ...
        <span className="wilson-v mono">{n}</span>
      </span>
      <span className="wilson-block">
        <span className="wilson-k">wr</span>
        <span className="wilson-v mono">{(winRate * 100).toFixed(0)}%</span>
      </span>
      <span className="wilson-block">
        <span className="wilson-k">wilson lb</span>
        <span className="wilson-v mono">{(lb * 100).toFixed(0)}%</span>
      </span>
...
function Sparkline({ data, color = "currentColor", h = 22, w = 84, fill = true }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: "block" }}>
      ...
      <path d={d} stroke={color} fill="none" strokeWidth="1.25" />
    </svg>
  );
}
  ...
  return (
    <svg width={size} height={size * 0.86} viewBox={`0 0 ${size} ${size * 0.86}`}>
      ...
      <path
        ...
        stroke={tone} strokeWidth="10" fill="none" strokeLinecap="round"
      />
...
function Radar({ pillars, size = 200 }) {
  // pillars: { technical, fundamental, catalyst, risk, edge }
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <polygon points={polyPts} fill="var(--copper-bg)" stroke="var(--copper)" strokeWidth="1.5" />
      {pts.map((p, i) => (
        <circle key={i} cx={p[0]} cy={p[1]} r="2.5" fill="var(--copper)" />
      ))}
      {labelPts.map((p, i) => (
...
function Cone({ lo, mid, hi, current = 0, w = 280, h = 110 }) {
  // implied-move cone — lo/mid/hi in %
  const pad = 14;
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: "block" }}>
      ...
      {/* 3σ band */}
      <path d={path(hi)} stroke="var(--gn)" strokeOpacity="0.4" strokeWidth="1.25" fill="none" />
      <path d={path(mid)} stroke="var(--gn)" strokeWidth="1.5" fill="none" />
      <path d={path(lo)} stroke="var(--rd)" strokeOpacity="0.6" strokeWidth="1.25" fill="none" />
      <circle cx={pad} cy={yMid} r="3" fill="var(--copper)" />
