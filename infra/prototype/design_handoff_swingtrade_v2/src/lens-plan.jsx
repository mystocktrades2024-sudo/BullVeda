...
function PayoffMini({ ticker }) {
  ...
  const w = 220, h = 60;
  const range = t2 - stop;
  const x = (p) => ((p - stop) / range) * w;
  return (
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="payoff-mini">
      <line x1="0" y1={h/2} x2={w} y2={h/2} stroke="var(--line)" />
      <line x1={x(stop)} y1="0" x2={x(stop)} y2={h} stroke="var(--rd-dim)" strokeDasharray="2 2" />
      <line x1={x(entry)} y1="0" x2={x(entry)} y2={h} stroke="var(--copper)" />
      <line x1={x(t1)} y1="0" x2={x(t1)} y2={h} stroke="var(--gn-dim)" strokeDasharray="2 2" />
      <line x1={x(t2)} y1="0" x2={x(t2)} y2={h} stroke="var(--gn-dim)" strokeDasharray="2 2" />
      {/* loss region */}
      <line x1={x(stop)} y1={h - 4} x2={x(entry)} y2={h/2} stroke="var(--rd)" strokeWidth="1.5" />
      {/* gain region to t2 */}
      <line x1={x(entry)} y1={h/2} x2={x(t2)} y2="6" stroke="var(--gn)" strokeWidth="1.5" />
      <text x={x(stop)} y={h - 6} fontSize="9" className="mono" fill="var(--rd)">${stop.toFixed(0)}</text>
      <text x={x(entry)} y={12} fontSize="9" className="mono" fill="var(--copper)" textAnchor="middle">${entry.toFixed(0)}</text>
      <text x={x(t2) - 4} y={12} fontSize="9" className="mono" fill="var(--gn)" textAnchor="end">${t2.toFixed(0)}</text>
    </svg>
  );
}
