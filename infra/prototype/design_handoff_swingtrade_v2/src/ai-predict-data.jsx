...
(function () {
  ...
  function predict(sym) {
    const meta = NAMES[sym] || [sym, 100, "Tech"];
    const seed = hash(sym) ^ 0x5eed;
    const r = mulberry(seed);
    const px = meta[1];
    // three model heads each produce a directional score in [-1,1]
  ...
  const ALL = Object.keys(NAMES).map(predict).sort((a, b) => b.score - a.score);
