...
// Logs every published call (8 sources), scores forward returns at 23 horizons
// (D1–D7, W1–W8, M1–M12), direction-aligned, both raw and edge-vs-SPY, with
// maturity awareness, regime tagging, and AI calibration. Exposes window.SigLedger.
...
(function () {
  ...
  const HZ_BY = {}; HZ.forEach((h, i) => { h.idx = i; HZ_BY[h.id] = h; });
  ...
  const SOURCE_BY = {}; SOURCES.forEach(s => SOURCE_BY[s.id] = s);
  ...
  const TODAY = 0; // age 0 = today; signals have positive age (days ago)
  ...
  SOURCES.forEach(src => {
    for (let k = 0; k < src.n; k++) {
      const age = Math.floor(Math.pow(rng(), 0.7) * 365) + 1; // skew toward recent, up to ~1y
      const sym = SYMS[Math.floor(rng() * SYMS.length)];
      const dir = rng() < src.dirBias ? "long" : "short";
      const regime = REGIMES[Math.floor(rng() * REGIMES.length)];
      const refPrice = +(15 + rng() * 400).toFixed(2);
      const predProb = src.hasProb ? +(0.5 + rng() * 0.45).toFixed(2) : null;
      const skill = (rng() - 0.42); // per-signal skill draw (centered slightly positive)
      SIGNALS.push({ id: "s" + (sid++), source: src.id, sym, dir, regime, age, refPrice, predProb, skill, seed: Math.floor(rng() * 1e9) });
    }
  });

  // mean aligned EDGE (%) for a source at a horizon — a decay curve peaking at src.peak
  function meanEdge(src, hz) {
    const g = src.amp * Math.exp(-Math.pow(hz.days - src.peak, 2) / (2 * src.breadth * src.breadth));
    const drift = -0.004 * hz.days; // slow bleed at long horizons
    return src.base + g + drift;
  }
  // SPY move (%) over a horizon (mild positive drift + small noise via seed)
  function spyMove(hz, seed) { const r = mulberry(seed ^ 0xABCD); return hz.days * 0.018 + gauss(r) * Math.sqrt(hz.days) * 0.35; }

  // per (signal, horizon) → { edge, raw, hit, mature } for a given metric handled by caller
  function ret(sig, hz) {
    const src = SOURCE_BY[sig.source];
    const mature = hz.days <= sig.age;
    const r = mulberry(sig.seed ^ (hz.idx * 2654435761));
    const idio = gauss(r) * src.vol;
    // skill boosts the aligned edge near the source's peak horizon
    ...
    const sM = spyMove(hz, sig.seed);
    const raw = sig.dir === "long" ? edge + sM : edge - sM;
  ...
  // full per-source summary across all horizons (for leaderboard / decay)
  function sourceSummary(sourceId, metric) {
    ...
    // headline = average across W1..W4 band (swing-relevant), fallback to all
    ...
    // optimal holding period = horizon with max mean (matured)
    let best = null; matured.forEach(d => { if (!best || d.mean > best.mean) best = d; });
    // profit factor across a representative horizon (W2) using individual signals
    const sigs = SIGNALS.filter(s => s.source === sourceId);
    const w2 = HZ_BY["W2"]; let gw = 0, gl = 0, nb = 0, wins = 0;
    const indiv = [];
    sigs.forEach(s => { const r = ret(s, w2); if (r.mature) { const v = metric === "raw" ? r.raw : r.edge; if (v > 0) { gw += v; wins++; } else gl += -v; nb++; indiv.push({ sym: s.sym, v }); } });
    const pf = gl > 0 ? +(gw / gl).toFixed(2) : (gw > 0 ? 99 : 0);
    indiv.sort((a, b) => b.v - a.v);
    const tstat = aggregate(sourceId, w2, metric).t;
    return { id: sourceId, label: SOURCE_BY[sourceId].label, decay, avgEdge, avgHit, best, pf, nTotal: sigs.length, nMatured: nb, best1: indiv[0], worst1: indiv[indiv.length - 1], tstat, sig: Math.abs(tstat) >= 2 };
  }

  function leaderboard(metric) {
    return SOURCES.map(s => sourceSummary(s.id, metric)).sort((a, b) => b.avgEdge - a.avgEdge);
  }

  // individual ledger rows enriched for a metric (with full horizon path)
  function ledger(metric, filter) {
    return SIGNALS.filter(s => !filter || !filter.source || s.source === filter.source)
      .map(s => {
        const path = HZ.map(hz => { const r = ret(s, hz); return { hz: hz.id, days: hz.days, v: metric === "raw" ? r.raw : r.edge, mature: r.mature }; });
        const mat = path.filter(p => p.mature);
        const last = mat.length ? mat[mat.length - 1] : null;
        return { ...s, label: SOURCE_BY[s.source].label, path, last, status: s.age < HZ_BY["M12"].days ? "maturing" : "matured", maturedN: mat.length };
      }).sort((a, b) => a.age - b.age);
  }

  // AI calibration: predicted prob bucket → realized hit-rate (at W2)
  function calibration(metric) {
    const w2 = HZ_BY["W2"]; const buckets = [[.5,.6],[.6,.7],[.7,.8],[.8,.9],[.9,1.01]];
    return buckets.map(([lo, hi]) => {
      const sigs = SIGNALS.filter(s => s.source === "ai" && s.predProb >= lo && s.predProb < hi);
  ...
  // regime hit-rate per source (at W2)
  function regimeHits(metric) {
    const w2 = HZ_BY["W2"]; const out = {};
  ...
  // equity curve: cumulative aligned return following every call of a source at horizon
  function equityCurve(sourceId, hzId, metric) {
    const hz = HZ_BY[hzId] || HZ_BY["W2"];
  ...
  window.SigLedger = {
    HZ, HZ_BY, SOURCES, SOURCE_BY, SIGNALS,
    aggregate, sourceSummary, leaderboard, ledger, calibration, regimeHits, equityCurve,
