...
function ThemeQuickPick({ value, onChange }) {
  ...
  return (
    <div className="tqp" ref={ref}>
      <button className="tqp-btn" onClick={() => setOpen(o => !o)} title={`Switch theme · current: ${current?.name || value}`}>
        <span className="tqp-current">
          {current && current.sw.slice(0, 3).map((c, i) => (
            <span key={i} className="tqp-chip" style={{ background: c }} />
          ))}
        </span>
        <span className="tqp-lbl mono">THEME</span>
        <span className="tqp-name mono">{current?.name || "Theme"}</span>
      ...
      {open && ReactDOM.createPortal((
        <div className="tqp-menu">
          ...
          <div className="tqp-menu-groups">
            {[
              { lbl: "CUSTOM · BESPOKE", test: t => t.custom },
              { lbl: "GLOSSY · GLASS", test: t => !t.custom && !t.id.startsWith("matte-") && t.id !== "paper-flat" && !t.id.startsWith("arctic") && !t.id.startsWith("ivory") },
