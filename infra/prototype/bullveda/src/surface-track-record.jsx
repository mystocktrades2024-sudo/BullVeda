...
function LedgerView({ SL, metric, srcFilter, setSrcFilter, onTicker }) {
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `track-record_${metric}_${srcFilter || "all"}_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="wsx-body">
      ...
      <div className="lab-verdict mono dim2">{rows.length} calls{srcFilter ? ` · ${SL.SOURCE_BY[srcFilter].label}` : ""} · {period === "all" ? "all history (≤1y)" : period === "ytd" ? "year-to-date" : "last " + period.toUpperCase()} · {metric === "edge" ? "edge vs SPY" : "raw return"}. <b>⤓ Export CSV</b> downloads the filtered set across all {NH} horizons. Click a row → ticker detail.</div>
    </div>
  );
}
