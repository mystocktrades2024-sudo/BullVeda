...
export function render() {
  const T = _T();
  const v = T.verdict || T.stage || 'WATCH';
  ...
  if (ctp) {
    ...
    if (ctp.target1 != null)     T.t1         = ctp.target1;
    if (ctp.target2 != null)     T.t2         = ctp.target2;
    if (ctp.entry?.low != null)  T.entry_lo   = ctp.entry.low;
    if (ctp.entry?.high != null) T.entry_hi   = ctp.entry.high;
    if (ctp.risk?.rr_ratio != null) T.rr      = ctp.risk.rr_ratio;
  }
  // Compute real R:R from CURRENT price (not cached entry_lo)
  const curPrice = T.price || T.entry_lo || 0;
  const realRR = (curPrice && T.stop && T.t1 && curPrice > T.stop && T.t1 > curPrice)
    ? (T.t1 - curPrice) / (curPrice - T.stop) : null;
  const cachedRR = T.rr || 0;
  const rrMismatch = realRR != null && cachedRR && Math.abs(realRR - cachedRR) / cachedRR > 0.15;
  const headlines = {
    BUY:   `<span class="accent">BUY</span> — entry zone live around $${px(T.entry_lo)}.`,
    WAIT:  `<span class="accent" style="color:#eab308">WAIT</span> — ${demoteReason}. Setup is real but entry timing is wrong.`,
    WATCH: `<span class="accent">WATCH</span> — wait for confirmation at $${px(T.entry_lo)}.`,
  ...
  const sub = `${T.setup || 'Setup'} · ${T.sector || ''}. Score ${T.score}/100 with ${T.tech_score || 0}+${T.fund_score || 0}+${T.sent_score || 0}+${T.smc_pts || T.smc_score || 0} pillar split. ${T.above_50ema ? 'Trend stacked above EMA50.' : 'Below EMA50 — context risk.'} ${T.earn_days != null && T.earn_days <= 14 ? `Earnings ${T.earn_days}d out — size to survive a miss.` : 'No imminent earnings.'}`;
  ...
  const actions = isDemoted ? [
    ...
    {n:4, what:`If price runs to $${px(T.t1)} without pullback: <b>missed it</b> — let it go`, meta:'DISCIPLINE · no FOMO entries', go:'PASS'},
  ] : [
    {n:1, what:`Enter limit at <b>$${px((T.entry_lo + T.entry_hi)/2)}</b> — inside entry zone $${px(T.entry_lo)}–$${px(T.entry_hi)}`, meta:`LIMIT · zone · ${cachedRR?.toFixed(1) || '—'} R:R restored`, go:'STAGE'},
    {n:2, what:`Hard stop at <b>$${px(T.stop)}</b> immediately on fill`, meta:'STOP · GTC · non-negotiable', go:'SET'},
    {n:3, what:`Trim 50% at T1 <b>$${px(T.t1)}</b>, move stop to BE`, meta:`LIMIT · lock partial · realize +${(((T.t1 - T.entry_lo)/(T.entry_lo - T.stop))).toFixed(1)}R`, go:'PLAN'},
