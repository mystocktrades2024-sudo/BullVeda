...
/* ─── qov4-* · v4 analytical-engine visuals (2026-05-12) ─────────────── */
.qov4-wrap{display:flex;flex-direction:column;gap:14px;margin-bottom:16px}
.qov4-panel{background:#0b0e14;border:1px solid #1e2530;border-radius:4px;padding:14px 16px}
.qov4-panel h3{font-family:var(--qmono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:#9aa4b2;font-weight:700;margin:0 0 10px;display:flex;align-items:baseline;gap:10px}
.qov4-panel h3 .qov4-num{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:3px;background:#1a2030;color:#5ea1ff;font-size:11px;font-weight:800}
.qov4-panel h3 .qov4-sub{color:#6b7280;font-size:10px;font-weight:500;letter-spacing:.04em;text-transform:none;font-style:italic}
.qov4-panel h3 .qov4-asof{margin-left:auto;color:#6b7280;font-size:10px;letter-spacing:.06em}
.qov4-stub{padding:14px 12px;border:1px dashed #2a3340;border-radius:3px;color:#6b7280;font-family:var(--qmono);font-size:11px;text-align:center}
.qov4-stub b{color:#9aa4b2}
.qov4-grid2{display:grid;grid-template-columns:1.4fr 1fr;gap:14px}
.qov4-headline{margin-top:10px;padding:9px 12px;background:rgba(94,161,255,0.07);border-left:2px solid #5ea1ff;color:#e6eaf2;font-size:12px;line-height:1.55;border-radius:0 3px 3px 0}
.qov4-conv-grid{display:grid;grid-template-columns:1.8fr 1fr;gap:14px}
.qov4-rules{display:flex;flex-direction:column;gap:6px}
.qov4-rule{display:grid;grid-template-columns:18px 1fr 56px;gap:10px;align-items:center;padding:8px 10px;background:#0f1311;border:1px solid #1e2530;border-radius:3px}
.qov4-rule.pass{border-left:2px solid #3DDC97}
.qov4-rule.fail{border-left:2px solid #E5484D}
.qov4-rule .qov4-rmark{font-size:13px;font-weight:800}
.qov4-rule.pass .qov4-rmark{color:#3DDC97}
.qov4-rule.fail .qov4-rmark{color:#E5484D}
.qov4-rule .qov4-rname{font-size:11.5px;color:#e6eaf2;font-weight:600}
.qov4-rule .qov4-revid{font-size:10px;color:#9aa4b2;font-family:var(--qmono);margin-top:2px}
.qov4-rule .qov4-rstamp{font-family:var(--qmono);font-size:9.5px;font-weight:800;text-align:center;padding:3px 6px;border-radius:2px}
.qov4-rule.pass .qov4-rstamp{background:rgba(61,220,151,0.12);color:#3DDC97;border:1px solid #2f6b41}
.qov4-rule.fail .qov4-rstamp{background:rgba(229,72,77,0.10);color:#E5484D;border:1px solid #7a2a22}
.qov4-tier{padding:16px;background:#0f1311;border:1px solid #1e2530;border-radius:3px;display:flex;flex-direction:column;gap:8px;justify-content:center;align-items:center}
.qov4-tier-k{font-family:var(--qmono);font-size:10px;letter-spacing:.14em;color:#6b7280;font-weight:700}
.qov4-tier-v{font-size:30px;font-weight:900;line-height:1}
.qov4-tier.HIGH .qov4-tier-v{color:#3DDC97}
.qov4-tier.MEDIUM .qov4-tier-v{color:#F5A524}
.qov4-tier.LOW .qov4-tier-v{color:#E5484D}
.qov4-tier-r{font-size:10.5px;color:#9aa4b2;font-family:var(--qmono)}
@media (max-width:1024px){.qov4-grid2,.qov4-conv-grid{grid-template-columns:1fr}}
`;
...
export function render() {
  ...
  // 2026-05-12 Mode-aware reads: when the SWING/POSITION/INVEST toggle is set
  // (window.qdMode → 'position' or 'invest'), prefer T.modes[mode] values
  // for stop/t1/t2 so the Plan Ladder + Pre-Flight actually recompute on toggle.
  // Falls through to swing-mode legacy fields if T.modes isn't present
  // (e.g. on-demand quick-dive payloads).
  const _qmode = (typeof window !== 'undefined' && typeof window.qdModeBlock === 'function')
                  ? window.qdModeBlock() : null;
  const _qmodeName = (typeof window !== 'undefined' && typeof window.qdMode === 'function')
                    ? window.qdMode() : 'swing';
  const stop = num(_qmode?.stop ?? ctp.stop ?? T.trade_plan?.stop ?? T.stop, 0);
  const t1   = num(_qmode?.t1   ?? ctp.target1 ?? T.trade_plan?.target1 ?? T.target1 ?? T.t1, 0);
  const t2   = num(_qmode?.t2   ?? ctp.target2 ?? T.trade_plan?.target2 ?? T.target2 ?? T.t2, 0);
    ...
    <div class="qov-grid2">
      <div class="qov-pane">
        ...
        <table class="qov-tbl">
          ...
          ${stop ? ladderRow('STOP', 'qov-rd', stop, (_qmodeName === 'position' ? '1.75× ATR · POSITION' : _qmodeName === 'invest' ? '−15% drawdown · INVEST' : '1.25× ATR · SWING'), stopPct, 'qov-rd') : ''}
        </table>
      </div>
      <div class="qov-pane">
  ...
  const checklist = [
    ...
    // 2026-05-12 Mode-aware thresholds — R:R floor, entry-quality allowlist
    // and ER window all read from T.modes[currentMode] when the toggle is set.
    // Falls through to legacy swing values (3.0 / FRESH+PULLBACK / 14d) when
    // T.modes isn't on the payload (on-demand quick-dive).
    (function(){
      const rrFloor = _qmode?.rr_floor ?? 3;
      return {
        n: 3, rule: `R:R ≥ ${rrFloor.toFixed(1)} *`,
        detail: !isNaN(rr) && rr ? `1:${rr.toFixed(1)} ${rr >= rrFloor ? '≥' : '&lt;'} ${rrFloor.toFixed(1)}` : '—',
        pass: rr >= rrFloor,
      };
    })(),
    (function(){
      const eqReq = _qmode?.entry_quality_req || ['FRESH','PULLBACK','VALID'];
      return {
        n: 4, rule: `Entry quality ${eqReq.slice(0,2).join('/')}${eqReq.length>2?'+':''} *`,
        detail: eqRaw || '—',
        pass: eqReq.includes(eqRaw),
      };
    })(),
    (function(){
      // ER window semantics differ by mode:
      //   swing:    er_window_days = 14  → require ER ≥14d clear (avoid binary)
      //   position: er_window_days = -30 → ER within 30d welcome (catalyst)
      //   invest:   er_window_days = null → irrelevant
      const w = _qmode?.er_window_days;
      const rule = _qmode?.er_rule || 'Earnings ≥ 14d clear';
      let pass, detail;
      if (w == null) {
        pass = true;
        detail = isNaN(earnDays) ? '—' : `+${earnDays}d · agnostic`;
      } else if (w > 0) {
        // Need ER FURTHER OUT than w days (avoid the binary)
        pass = isNaN(earnDays) || earnDays >= w || earnDays < 0;
        detail = isNaN(earnDays) ? 'no earnings in window' : `+${earnDays}d`;
      } else {
        // w < 0 means we WANT ER within |w| days (catalyst welcome)
        const limit = -w;
        pass = !isNaN(earnDays) && earnDays >= 0 && earnDays <= limit;
        detail = isNaN(earnDays) ? 'no upcoming ER' : `+${earnDays}d · target ≤${limit}d`;
      }
      return { n: 5, rule: `${rule} *`, detail, pass };
    })(),
    {
      n: 6, rule: 'ADV ≥ $5M',
  ...
  // ── ★ V4 ANALYTICAL-ENGINE VISUALS (2026-05-12) ★ ──────────────────
  // 9 visuals from cache/analytical_engine_v4_prototype.html, wired to REAL data.
  // Strategy is read from window._qdStrategy (set by sibling qdSetStrategy()).
  const _strategy = (typeof window !== 'undefined' && window._qdStrategy)
                    ? String(window._qdStrategy).toUpperCase()
                    : 'POSITION';
  const _escHtml = (s) => String(s == null ? '' : s)
        .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  const _stub = (label, reason) =>
    `<div class="qov4-stub"><b>${_escHtml(label)}</b> — data unavailable · ${_escHtml(reason)}</div>`;

  // verdict-card data (real)
  const _vdCallTxt = v;
  const _vdCallColor = (v === 'BUY') ? '#3DDC97'
                     : (v === 'SHORT' || v === 'AVOID' || v === 'SELL') ? '#E5484D'
                     : '#F5A524';
  const _vdGradId = (v === 'BUY') ? 'vdGradGN'
                   : (v === 'SHORT' || v === 'AVOID' || v === 'SELL') ? 'vdGradRD'
                   : 'vdGradAM';
  const _vdHorizon = ({SWING:'2–10 days', POSITION:'4–6 weeks', INVESTMENT:'6–12 months'})[_strategy] || '—';

  // edge sign — real if monte_carlo present
  const _edgePp = !isNaN(edge) ? (edge * 100) : NaN;
  const _edgeTxt = !isNaN(_edgePp) ? (_edgePp >= 0 ? '+' : '') + _edgePp.toFixed(0) + 'pp' : '—';
  const _edgeColor = isNaN(_edgePp) ? '#9aa4b2' : (_edgePp >= 0 ? '#3DDC97' : '#E5484D');
  const _pT1Pct = !isNaN(pT1First) ? (pT1First * 100).toFixed(0) + '%' : '—';

  // R:R real
  const _rrTxt = (!isNaN(rr) && rr > 0) ? rr.toFixed(2) + 'R' : '—';
  const _rrColor = (rr >= 2) ? '#3DDC97' : (rr >= 1) ? '#F5A524' : '#E5484D';
  const _rewardMove = (!isNaN(t1) && !isNaN(pX) && pX > 0) ? (t1 - pX) : NaN;
  const _riskMove   = (!isNaN(stop) && !isNaN(pX) && pX > 0) ? (stop - pX) : NaN;
  const _rrSub = (!isNaN(_rewardMove) && !isNaN(_riskMove))
    ? `+$${_rewardMove.toFixed(2)} reward vs $${_riskMove.toFixed(2)} risk` : '—';

  // sizing real
  const _sizeTxt = !isNaN(finalAlloc) ? finalAlloc.toFixed(1) + '%' : '—';
  const _sizeColor = isNaN(finalAlloc) ? '#9aa4b2' : (finalAlloc > 0 ? '#5EA1FF' : '#6b7280');
  const _sizeMultParts = [];
  if (!isNaN(regimeMult)) _sizeMultParts.push(`regime ${regimeMult.toFixed(2)}×`);
  if (!isNaN(ddMult))     _sizeMultParts.push(`DD ${ddMult.toFixed(2)}×`);
  if (!isNaN(vixMult))    _sizeMultParts.push(`VIX ${vixMult.toFixed(2)}×`);
  const _sizeSub = _sizeMultParts.length ? _sizeMultParts.join(' · ') : 'Kelly-lite';

  // CVaR
  const _cvarTxt = !isNaN(cvar975) ? (cvar975 >= 0 ? '+' : '') + cvar975.toFixed(1) + '%' : '—';
  const _cvarColor = isNaN(cvar975) ? '#9aa4b2' : '#E5484D';

  // conviction arc — real, score-based (0–100)
  const _convPct = Math.max(0, Math.min(100, score));
  const _convTier = (_convPct >= 80) ? 'HIGH' : (_convPct >= 60) ? 'MEDIUM' : 'LOW';
  const _convArcColor = (_convTier === 'HIGH') ? '#3DDC97' : (_convTier === 'MEDIUM') ? '#F5A524' : '#E5484D';

  // lens votes — derive from pillar scores (techS / catS / rsS+smS / qS)
  const _voteOf = (pts, max) => {
    if (!max || isNaN(pts)) return { txt:'—', color:'#6b7280' };
    const pct = pts / max;
    if (pct >= 0.70) return { txt:'BUY', color:'#3DDC97' };
    if (pct >= 0.45) return { txt:'HOLD', color:'#F5A524' };
    return { txt:'SELL', color:'#E5484D' };
  };
  const _voteT = _voteOf(techS, techMax);
  const _voteH = _voteOf(catS + smS, 20 + 15);
  const _voteV = _voteOf(qS, fundMax);
  const _voteP = _voteOf(rsS, 20);

  // headline 1-liner
  const _vdHeadline = `${_vdCallTxt} call · ${_strategy} window · score <b>${score}</b> · entry <b>${eqRaw || '—'}</b>${!isNaN(_edgePp) ? ` · quant edge <b style="color:${_edgeColor}">${_edgeTxt}</b>` : ''}`;

  // === ⓥ VERDICT CARD ===
  const _verdictHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">V</span>VERDICT <span class="qov4-sub">one-look summary · strategy-aware</span><span class="qov4-asof">${_strategy} · ${_vdHorizon}</span></h3>
      <svg viewBox="0 0 1200 220" style="width:100%;height:auto;display:block">
        <defs>
          <linearGradient id="qov4-vdGradGN" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#3DDC97" stop-opacity=".45"/><stop offset="1" stop-color="#3DDC97" stop-opacity=".10"/></linearGradient>
          <linearGradient id="qov4-vdGradAM" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#F5A524" stop-opacity=".45"/><stop offset="1" stop-color="#F5A524" stop-opacity=".10"/></linearGradient>
          <linearGradient id="qov4-vdGradRD" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#E5484D" stop-opacity=".45"/><stop offset="1" stop-color="#E5484D" stop-opacity=".10"/></linearGradient>
          <linearGradient id="qov4-vdConvArc" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#E5484D"/><stop offset=".5" stop-color="#F5A524"/><stop offset="1" stop-color="#3DDC97"/></linearGradient>
        </defs>
        <g transform="translate(20,30)">
          <rect width="245" height="160" rx="6" fill="url(#qov4-${_vdGradId})" stroke="${_vdCallColor}" stroke-width="1.6"/>
          <text x="122" y="26" text-anchor="middle" font-family="monospace" font-size="10" fill="#9aa4b2" font-weight="700" letter-spacing=".16em">VERDICT</text>
          <text id="qov4-vdCall" x="122" y="84" text-anchor="middle" font-family="-apple-system,sans-serif" font-size="52" font-weight="900" fill="${_vdCallColor}" letter-spacing=".04em">${_vdCallTxt}</text>
          <text x="122" y="112" text-anchor="middle" font-size="12.5" fill="#e6eaf2" font-weight="700">${_strategy} · ${_vdHorizon}</text>
          <text x="122" y="132" text-anchor="middle" font-size="10.5" fill="#9aa4b2">entry ${eLo && eHi ? '$' + eLo.toFixed(2) + '–$' + eHi.toFixed(2) : '—'}</text>
          <text x="122" y="150" text-anchor="middle" font-family="monospace" font-size="9.5" fill="#9aa4b2">P(T1) ${_pT1Pct}${!isNaN(setupN_hist) ? ` · n=${setupN_hist}` : ''}</text>
        </g>
        <g transform="translate(285,30)">
          <g>
            <rect width="245" height="74" rx="5" fill="rgba(22,27,36,.6)" stroke="#1e2530"/>
            <text x="14" y="20" font-family="monospace" font-size="9.5" fill="#9aa4b2" font-weight="700" letter-spacing=".14em">QUANT EDGE</text>
            <text x="14" y="50" font-size="26" font-weight="800" fill="${_edgeColor}">${_edgeTxt}</text>
            <text x="14" y="66" font-size="10" fill="#9aa4b2">MC P(T1)−P(stop) · ${!isNaN(pT1First) ? 'real' : 'no monte_carlo'}</text>
          </g>
          <g transform="translate(260,0)">
            <rect width="245" height="74" rx="5" fill="rgba(22,27,36,.6)" stroke="#1e2530"/>
            <text x="14" y="20" font-family="monospace" font-size="9.5" fill="#9aa4b2" font-weight="700" letter-spacing=".14em">R:R RATIO</text>
            <text x="14" y="50" font-size="26" font-weight="800" fill="${_rrColor}">${_rrTxt}</text>
            <text x="14" y="66" font-size="10" fill="#9aa4b2">${_escHtml(_rrSub)}</text>
          </g>
          <g transform="translate(0,82)">
            <rect width="245" height="74" rx="5" fill="rgba(22,27,36,.6)" stroke="#1e2530"/>
            <text x="14" y="20" font-family="monospace" font-size="9.5" fill="#9aa4b2" font-weight="700" letter-spacing=".14em">POSITION SIZE</text>
            <text x="14" y="50" font-size="26" font-weight="800" fill="${_sizeColor}">${_sizeTxt}</text>
            <text x="14" y="66" font-size="10" fill="#9aa4b2">${_escHtml(_sizeSub)}</text>
          </g>
          <g transform="translate(260,82)">
            <rect width="245" height="74" rx="5" fill="rgba(22,27,36,.6)" stroke="#1e2530"/>
            <text x="14" y="20" font-family="monospace" font-size="9.5" fill="#9aa4b2" font-weight="700" letter-spacing=".14em">TAIL RISK · CVaR</text>
            <text x="14" y="50" font-size="26" font-weight="800" fill="${_cvarColor}">${_cvarTxt}</text>
            <text x="14" y="66" font-size="10" fill="#9aa4b2">${!isNaN(cvar975) ? '10d · 97.5% tail' : 'no fwd_dist data'}</text>
          </g>
        </g>
        <g transform="translate(820,30)">
          <rect width="360" height="160" rx="6" fill="rgba(22,27,36,.6)" stroke="#1e2530"/>
          <text x="180" y="22" text-anchor="middle" font-family="monospace" font-size="10" fill="#9aa4b2" font-weight="700" letter-spacing=".16em">CONVICTION</text>
          <path d="M 60 110 A 120 120 0 0 1 300 110" fill="none" stroke="#1e2530" stroke-width="14" stroke-linecap="round"/>
          <path id="qov4-vdConvArcFill" d="M 60 110 A 120 120 0 0 1 300 110" fill="none" stroke="url(#qov4-vdConvArc)" stroke-width="14" stroke-linecap="round" pathLength="100" stroke-dasharray="${_convPct} 100"/>
          <text x="180" y="98" text-anchor="middle" font-size="26" font-weight="900" fill="${_convArcColor}">${_convTier}</text>
          <text x="180" y="118" text-anchor="middle" font-family="monospace" font-size="11" fill="#9aa4b2" font-weight="700">${_convPct.toFixed(0)} / 100</text>
          <g transform="translate(20,135)" font-family="monospace" font-size="9.5">
            <text x="0" y="0" fill="#9aa4b2" font-weight="700" letter-spacing=".10em">LENS VOTES</text>
            <g transform="translate(95,-9)">
              <g transform="translate(0,0)"><rect width="58" height="14" rx="2" fill="${_voteT.color}22" stroke="${_voteT.color}"/><text x="29" y="10" text-anchor="middle" fill="${_voteT.color}" font-weight="800">T ${_voteT.txt}</text></g>
              <g transform="translate(66,0)"><rect width="58" height="14" rx="2" fill="${_voteH.color}22" stroke="${_voteH.color}"/><text x="29" y="10" text-anchor="middle" fill="${_voteH.color}" font-weight="800">H ${_voteH.txt}</text></g>
              <g transform="translate(132,0)"><rect width="58" height="14" rx="2" fill="${_voteV.color}22" stroke="${_voteV.color}"/><text x="29" y="10" text-anchor="middle" fill="${_voteV.color}" font-weight="800">V ${_voteV.txt}</text></g>
              <g transform="translate(198,0)"><rect width="58" height="14" rx="2" fill="${_voteP.color}22" stroke="${_voteP.color}"/><text x="29" y="10" text-anchor="middle" fill="${_voteP.color}" font-weight="800">P ${_voteP.txt}</text></g>
            </g>
          </g>
        </g>
      </svg>
      <div class="qov4-headline">${_vdHeadline}</div>
    </div>`;

  // === ① DATA QUALITY STRIP ===
  // 10 cells: PRICE, MCAP, ADV, BETA, FWD P/E, PEG, SHORT %, IMPL ±, FCF MGN, ROE
  const _dqCells = [
    { k:'PRICE',    v:!isNaN(pX) && pX ? '$'+pX.toFixed(2) : null, src:'Schwab', age:'<15 min' },
    { k:'MCAP',     v:!isNaN(mcap) ? fmtMcap(mcap) : null,         src:'EODHD',  age:'1d' },
    { k:'ADV',      v:!isNaN(advDol) ? fmtMcap(advDol) : null,     src:'EODHD',  age:'12h' },
    { k:'BETA',     v:!isNaN(beta) ? beta.toFixed(2) : null,       src:'EODHD',  age:'12h' },
    { k:'FWD P/E',  v:!isNaN(fwdPE) ? fwdPE.toFixed(1) : null,     src:'EODHD',  age:'1d' },
    { k:'PEG',      v:!isNaN(peg) ? peg.toFixed(2) : null,         src:'EODHD',  age:'1d' },
    { k:'SHORT %',  v:!isNaN(shortPct) ? shortPct.toFixed(1)+'%':null, src:'Finviz', age:'1d' },
    { k:'IMPL ±',   v:!isNaN(impMove) ? '±'+impMove.toFixed(1)+'%':null, src:'Schwab', age:'2h' },
    { k:'FCF MGN',  v:!isNaN(fcfMgn) ? (fcfMgn>=0?'+':'')+fcfMgn.toFixed(1)+'%':null, src:'EODHD', age:'1d' },
    { k:'ROE',      v:!isNaN(roe) ? roe.toFixed(1)+'%' : null,     src:'EODHD',  age:'1d' },
  ];
  const _dqMissing = _dqCells.filter(c => c.v == null).length;
  const _dqCellsSvg = _dqCells.map((c, i) => {
    const x = i * 101;
    const filled = c.v != null;
    const fill = filled ? 'rgba(61,220,151,0.12)' : 'rgba(229,72,77,0.10)';
    const stroke = filled ? '#3DDC97' : '#E5484D';
    const kColor = filled ? '#3DDC97' : '#E5484D';
    return `<g transform="translate(${x},0)">
      <rect width="93" height="62" rx="4" fill="${fill}" stroke="${stroke}"/>
      <text x="46.5" y="14" text-anchor="middle" fill="${kColor}" font-size="9.5" font-weight="700">${c.k}</text>
      <text x="46.5" y="32" text-anchor="middle" fill="#e6eaf2" font-size="13" font-weight="700">${filled ? _escHtml(c.v) : '—'}</text>
      <text x="46.5" y="47" text-anchor="middle" fill="#9aa4b2" font-size="8">${_escHtml(c.src)}</text>
      <text x="46.5" y="57" text-anchor="middle" fill="#6b7280" font-size="7.5">${filled ? _escHtml(c.age) : 'missing'}</text>
    </g>`;
  }).join('');
  const _dqStatusColor = (_dqMissing === 0) ? '#3DDC97' : (_dqMissing <= 2) ? '#F5A524' : '#E5484D';
  const _dqHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">1</span>Data Quality Strip <span class="qov4-sub">gates all other visuals · provider + freshness</span></h3>
      <svg viewBox="0 0 1100 200" style="width:100%;height:auto;display:block" font-family="monospace">
        <text x="0" y="14" font-family="sans-serif" font-size="11" fill="#9aa4b2" font-weight="700">10 required inputs · ${10-_dqMissing}/10 valid</text>
        <g transform="translate(0,24)">
          ${_dqCellsSvg}
          <g transform="translate(1010,0)">
            <rect width="90" height="62" rx="4" fill="rgba(61,220,151,0.06)" stroke="#1e2530"/>
            <text x="45" y="14" text-anchor="middle" fill="#9aa4b2" font-size="9.5" font-weight="700">MISSING</text>
            <text x="45" y="36" text-anchor="middle" fill="${_dqStatusColor}" font-size="18" font-weight="800">${_dqMissing}/10</text>
            <text x="45" y="52" text-anchor="middle" fill="#6b7280" font-size="7.5">${_dqMissing===0?'no gaps':_dqMissing<=2?'minor':'high gap'}</text>
          </g>
        </g>
        <g transform="translate(0,95)" stroke="#2a3340" stroke-width=".7" fill="none">
          <path d="M 46.5 0 V 18 H 600 V 32" stroke="#3DDC97"/>
          <path d="M 147.5 0 V 22 H 920 V 32" stroke="#5EA1FF"/>
          <path d="M 248.5 0 V 16 H 920 V 32" stroke="#5EA1FF"/>
          <path d="M 349.5 0 V 20 H 260 V 32" stroke="#F5A524"/>
          <path d="M 450.5 0 V 14 H 30 V 32" stroke="#F5A524"/>
          <path d="M 551.5 0 V 14 H 30 V 32" stroke="#F5A524"/>
          <path d="M 652.5 0 V 18 H 460 V 32" stroke="#C084FC"/>
          <path d="M 753.5 0 V 16 H 800 V 32" stroke="#E5484D"/>
          <path d="M 854.5 0 V 20 H 30 V 32" stroke="#F5A524"/>
          <path d="M 955.5 0 V 22 H 30 V 32" stroke="#F5A524"/>
        </g>
        <g transform="translate(0,127)" font-family="monospace" font-size="9" font-weight="700" letter-spacing=".06em">
          <g transform="translate(0,0)"><rect width="120" height="20" rx="3" fill="rgba(245,165,36,0.10)" stroke="#F5A524"/><text x="60" y="14" text-anchor="middle" fill="#F5A524">→ VALUE LENS</text></g>
          <g transform="translate(200,0)"><rect width="120" height="20" rx="3" fill="rgba(245,165,36,0.10)" stroke="#F5A524"/><text x="60" y="14" text-anchor="middle" fill="#F5A524">→ LENS RADAR</text></g>
          <g transform="translate(430,0)"><rect width="120" height="20" rx="3" fill="rgba(192,132,252,0.10)" stroke="#C084FC"/><text x="60" y="14" text-anchor="middle" fill="#C084FC">→ HEDGE-FUND</text></g>
          <g transform="translate(570,0)"><rect width="120" height="20" rx="3" fill="rgba(61,220,151,0.10)" stroke="#3DDC97"/><text x="60" y="14" text-anchor="middle" fill="#3DDC97">→ PRICE CHART</text></g>
          <g transform="translate(770,0)"><rect width="120" height="20" rx="3" fill="rgba(229,72,77,0.10)" stroke="#E5484D"/><text x="60" y="14" text-anchor="middle" fill="#E5484D">→ R:R BAR</text></g>
          <g transform="translate(890,0)"><rect width="120" height="20" rx="3" fill="rgba(94,161,255,0.10)" stroke="#5EA1FF"/><text x="60" y="14" text-anchor="middle" fill="#5EA1FF">→ PM SIZING</text></g>
        </g>
        <text x="0" y="180" font-family="sans-serif" font-size="10" fill="#6b7280">Cells: <tspan fill="#3DDC97">valid</tspan> · <tspan fill="#E5484D">missing</tspan>. ≥3 missing/implausible → engine returns NO_CALL.</text>
      </svg>
    </div>`;

  // === ② LENS RADAR ===
  // Normalize each pillar 0–10
  const _norm10 = (pts, max) => (!max || isNaN(pts)) ? NaN : Math.max(0, Math.min(10, pts/max*10));
  const _rT = _norm10(techS, techMax);
  const _rH = _norm10(catS + smS, 20 + 15);
  const _rV = _norm10(qS, fundMax);
  const _rP = _norm10(rsS, 20);
  // axis tip-coords function from center (180,160), max-r=120
  const _v4RadarPt = (val, ang) => {
    if (isNaN(val)) return { x:180, y:160 };
    const r = (val/10) * 120;
    return { x:180 + Math.cos(ang)*r, y:160 + Math.sin(ang)*r };
  };
  // T=top(-PI/2), H=right(0), V=bottom(PI/2), P=left(PI)
  const _v4pT = _v4RadarPt(_rT, -Math.PI/2);
  const _v4pH = _v4RadarPt(_rH, 0);
  const _v4pVb = _v4RadarPt(_rV, Math.PI/2);
  const _v4pP = _v4RadarPt(_rP, Math.PI);
  const _v4RadarPoly = `${_v4pT.x},${_v4pT.y} ${_v4pH.x},${_v4pH.y} ${_v4pVb.x},${_v4pVb.y} ${_v4pP.x},${_v4pP.y}`;
  // Wilson-LB inset — use setup history WR if available
  const _wilsonScale = (setupLB_all != null) ? Math.min(1, Math.max(0.3, setupLB_all)) : 0.7;
  const _v4wPt = (p) => ({ x: 180 + (p.x-180)*_wilsonScale, y: 160 + (p.y-160)*_wilsonScale });
  const _v4w = [_v4wPt(_v4pT), _v4wPt(_v4pH), _v4wPt(_v4pVb), _v4wPt(_v4pP)];
  const _v4WilsonPoly = `${_v4w[0].x},${_v4w[0].y} ${_v4w[1].x},${_v4w[1].y} ${_v4w[2].x},${_v4w[2].y} ${_v4w[3].x},${_v4w[3].y}`;
  const _allLensNaN = isNaN(_rT) && isNaN(_rH) && isNaN(_rV) && isNaN(_rP);
  const _lensRadarHTML = _allLensNaN ? _stub('LENS RADAR', 'no pillar scores on T') : `
    <svg viewBox="0 0 360 360" style="width:100%;height:auto;display:block">
      <g stroke="#1e2530" fill="none" stroke-width=".7">
        <polygon points="180,40 300,160 180,280 60,160"/>
        <polygon points="180,70 270,160 180,250 90,160"/>
        <polygon points="180,100 240,160 180,220 120,160"/>
        <polygon points="180,130 210,160 180,190 150,160"/>
      </g>
      <g stroke="#2a3340" stroke-width=".8">
        <line x1="180" y1="160" x2="180" y2="40"/><line x1="180" y1="160" x2="300" y2="160"/>
        <line x1="180" y1="160" x2="180" y2="280"/><line x1="180" y1="160" x2="60" y2="160"/>
      </g>
      <polygon points="${_v4WilsonPoly}" fill="none" stroke="#5EA1FF" stroke-width="1" stroke-dasharray="3 2" opacity=".65"/>
      <polygon id="qov4-radarPoly" points="${_v4RadarPoly}" fill="rgba(94,161,255,.20)" stroke="#5EA1FF" stroke-width="1.8"/>
      <circle cx="${_v4pT.x}" cy="${_v4pT.y}" r="3.5" fill="#5EA1FF"/>
      <circle cx="${_v4pH.x}" cy="${_v4pH.y}" r="3.5" fill="#3DDC97"/>
      <circle cx="${_v4pVb.x}" cy="${_v4pVb.y}" r="3.5" fill="#F5A524"/>
      <circle cx="${_v4pP.x}" cy="${_v4pP.y}" r="3.5" fill="#3DDC97"/>
      <g font-family="sans-serif" font-size="11" fill="#e6eaf2">
        <text x="180" y="30" text-anchor="middle" fill="#5EA1FF" font-weight="700">TRADER (lead)</text>
        <text x="180" y="46" text-anchor="middle" font-family="monospace" font-size="16" fill="#5EA1FF" font-weight="800">${isNaN(_rT)?'—':_rT.toFixed(1)}</text>
        <text x="312" y="158" text-anchor="start">HEDGE FUND</text>
        <text x="312" y="174" text-anchor="start" font-family="monospace" font-size="16" fill="#3DDC97" font-weight="800">${isNaN(_rH)?'—':_rH.toFixed(1)}</text>
        <text x="180" y="312" text-anchor="middle">VALUE</text>
        <text x="180" y="296" text-anchor="middle" font-family="monospace" font-size="16" fill="#F5A524" font-weight="800">${isNaN(_rV)?'—':_rV.toFixed(1)}</text>
        <text x="48" y="158" text-anchor="end">PM</text>
        <text x="48" y="174" text-anchor="end" font-family="monospace" font-size="16" fill="#3DDC97" font-weight="800">${isNaN(_rP)?'—':_rP.toFixed(1)}</text>
      </g>
      <g font-family="monospace" font-size="9" fill="#6b7280">
        <text x="50" y="350">solid = score · dashed = Wilson LB${setupLB_all!=null?` (${(setupLB_all*100).toFixed(0)}%)`:''}</text>
      </g>
    </svg>`;

  // === ③ CATALYST HORIZON ===
  const _horizonDays = ({SWING:14, POSITION:56, INVESTMENT:180})[_strategy] || 56;
  const _catERX = !isNaN(earnDays) && earnDays >= 0
    ? Math.min(510, 30 + (earnDays / 150) * 480) : null;
  const _catBandW = Math.min(480, (_horizonDays/150) * 480);
  const _catERInside = _catERX != null && earnDays <= _horizonDays;
  const _catHorizonHTML = `
    <svg viewBox="0 0 540 320" style="width:100%;height:auto;display:block">
      <rect x="30" y="140" width="${_catBandW}" height="40" fill="rgba(94,161,255,.10)" stroke="#5EA1FF" stroke-width="1" rx="3"/>
      <text x="${30 + _catBandW/2}" y="135" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#5EA1FF" font-weight="700">${_strategy} window · ${_horizonDays}d</text>
      <rect x="${30+_catBandW}" y="140" width="${510-30-_catBandW}" height="40" fill="rgba(107,114,128,.06)" stroke="#1e2530" stroke-width="1" rx="3"/>
      <text x="${(30+_catBandW + 510)/2}" y="135" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#6b7280">out of horizon · context only</text>
      <line x1="30" y1="160" x2="510" y2="160" stroke="#2a3340" stroke-width=".8"/>
      <line x1="30" y1="120" x2="30" y2="200" stroke="#e6eaf2" stroke-width="1.5"/>
      <text x="30" y="115" text-anchor="middle" font-family="monospace" font-size="9" fill="#e6eaf2">TODAY</text>
      ${_catERX != null ? `
        <circle cx="${_catERX}" cy="160" r="6" fill="${_catERInside?'#3DDC97':'#6b7280'}" stroke="#0b0e14" stroke-width="2"/>
        <line x1="${_catERX}" y1="100" x2="${_catERX}" y2="154" stroke="${_catERInside?'#3DDC97':'#6b7280'}" stroke-width=".8" stroke-dasharray="2 2"/>
        <text x="${_catERX}" y="90" text-anchor="middle" font-family="sans-serif" font-size="10" fill="${_catERInside?'#3DDC97':'#6b7280'}" font-weight="700">Earnings</text>
        <text x="${_catERX}" y="78" text-anchor="middle" font-family="monospace" font-size="9" fill="${_catERInside?'#3DDC97':'#6b7280'}">+${earnDays}d</text>
        <text x="${_catERX}" y="183" text-anchor="middle" font-family="monospace" font-size="8.5" fill="#9aa4b2">${_catERInside?'in-window':'out of horizon'}</text>
      ` : `<text x="270" y="160" text-anchor="middle" font-family="monospace" font-size="10" fill="#6b7280">no earnings date</text>`}
      <g font-family="monospace" font-size="9" fill="#6b7280">
        <text x="30" y="260" text-anchor="middle">d0</text>
        <text x="126" y="260" text-anchor="middle">d30</text>
        <text x="222" y="260" text-anchor="middle">d60</text>
        <text x="318" y="260" text-anchor="middle">d90</text>
        <text x="414" y="260" text-anchor="middle">d120</text>
        <text x="510" y="260" text-anchor="middle">d150</text>
      </g>
      <text x="270" y="290" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#6b7280">${_strategy} rule: catalyst counts only if ≤ ${_horizonDays}d. Out-of-horizon events = context, not catalysts.</text>
    </svg>`;

  const _radarHorizonHTML = `
    <div class="qov4-grid2">
      <div class="qov4-panel">
        <h3><span class="qov4-num">2</span>Lens Radar <span class="qov4-sub">score (filled) · Wilson LB (dashed) · 4-axis</span></h3>
        ${_lensRadarHTML}
      </div>
      <div class="qov4-panel">
        <h3><span class="qov4-num">3</span>Catalyst Horizon <span class="qov4-sub">${_strategy} window · ER positioning</span></h3>
        ${_catHorizonHTML}
      </div>
    </div>`;

  // === ④ DISAGREEMENT MATRIX ===
  // 4 lenses (TRADER/HEDGE/VALUE/PM) × 6 signals (DIRECTION/REGIME/EDGE/CAT/LIQ/SIZING)
  // Build from real data; cell = 'BUY'|'HOLD'|'SELL'|'NA'
  const _dirVote = _voteT.txt; // trader's directional vote leads
  const _regimeOk = (regime === 'risk_on_trending' || regime === 'risk_on_choppy');
  const _edgeSign = !isNaN(_edgePp) ? (_edgePp > 0 ? 'BUY' : _edgePp < 0 ? 'SELL' : 'HOLD') : 'NA';
  const _catInHor = (_catERX != null && _catERInside) ? 'BUY' : (_catERX != null ? 'HOLD' : 'NA');
  const _liqOk = !isNaN(advDol) ? (advDol >= 10e6 ? 'BUY' : 'HOLD') : 'NA';
  const _sizingOk = !isNaN(finalAlloc) ? (finalAlloc > 0 ? 'BUY' : 'SELL') : 'NA';
  const _dmRows = [
    { lens:'TRADER',   color:'#5EA1FF', cells:[_voteT.txt, _regimeOk?'BUY':'SELL', _edgeSign, _catInHor, _liqOk, _sizingOk], w: _strategy==='SWING'?'1.4×':'1.0×' },
    { lens:'HEDGE',    color:'#3DDC97', cells:[_voteH.txt, _regimeOk?'BUY':'SELL', _edgeSign, _catInHor, _liqOk, _sizingOk], w: '1.0×' },
    { lens:'VALUE',    color:'#F5A524', cells:[_voteV.txt, 'HOLD',                  _edgeSign, _catInHor, _liqOk, _sizingOk], w: _strategy==='INVESTMENT'?'1.3×':'0.8×' },
    { lens:'PM',       color:'#3DDC97', cells:[_voteP.txt, _regimeOk?'BUY':'HOLD',  _edgeSign, _catInHor, _liqOk, _sizingOk], w: '1.0×' },
  ];
  const _dmColor = (cell) => cell==='BUY'?'#3DDC97':cell==='SELL'?'#E5484D':cell==='HOLD'?'#F5A524':'#6b7280';
  const _dmRowsSvg = _dmRows.map((r, ri) => {
    const y = 30 + ri*32;
    const cellsSvg = r.cells.map((c, ci) => {
      const cx = 155 + ci*140;
      const col = _dmColor(c);
      return `<rect x="${cx}" y="${y}" width="130" height="24" rx="2" fill="${col}22" stroke="${col}"/>
              <text x="${cx+65}" y="${y+15}" text-anchor="middle" font-size="10.5" font-weight="800" fill="${col}">${c}</text>`;
    }).join('');
    return `<g>
      <rect x="20" y="${y}" width="125" height="24" rx="2" fill="rgba(11,14,20,.7)" stroke="${r.color}"/>
      <text x="32" y="${y+15}" font-size="10" fill="${r.color}" font-weight="800">${r.lens}</text>
      ${cellsSvg}
      <text x="1070" y="${y+15}" text-anchor="middle" font-size="10.5" font-weight="700" fill="#5EA1FF">${r.w}</text>
    </g>`;
  }).join('');
  // agreement % per column
  const _colAgreeRow = [0,1,2,3,4,5].map(ci => {
    const cellVals = _dmRows.map(r => r.cells[ci]);
    const nonNa = cellVals.filter(v => v !== 'NA');
    if (!nonNa.length) return { pct:'—', color:'#6b7280' };
    const mode = nonNa.sort((a,b)=>nonNa.filter(v=>v===a).length - nonNa.filter(v=>v===b).length).pop();
    const agree = nonNa.filter(v=>v===mode).length / nonNa.length;
    const pct = (agree*100).toFixed(0)+'%';
    const color = agree >= 0.75 ? '#3DDC97' : agree >= 0.5 ? '#F5A524' : '#E5484D';
    return { pct, color };
  });
  const _agreeSvg = _colAgreeRow.map((a, ci) => {
    const cx = 155 + ci*140;
    return `<text x="${cx+65}" y="186" text-anchor="middle" font-size="11" font-weight="800" fill="${a.color}">${a.pct}</text>`;
  }).join('');
  const _disagreementHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">4</span>Disagreement Matrix <span class="qov4-sub">4 lenses × 6 signals · agreement % per signal · weights re-scale per strategy</span></h3>
      <svg viewBox="0 0 1180 260" style="width:100%;height:auto;display:block" font-family="monospace">
        <g font-size="9" fill="#9aa4b2" font-weight="700" letter-spacing=".10em">
          <text x="225" y="22" text-anchor="middle">DIRECTION</text>
          <text x="365" y="22" text-anchor="middle">REGIME</text>
          <text x="505" y="22" text-anchor="middle">EDGE SIGN</text>
          <text x="645" y="22" text-anchor="middle">CAT IN HORIZON</text>
          <text x="785" y="22" text-anchor="middle">LIQUIDITY</text>
          <text x="925" y="22" text-anchor="middle">SIZING</text>
          <text x="1070" y="22" text-anchor="middle" fill="#5EA1FF">WEIGHT (×)</text>
        </g>
        ${_dmRowsSvg}
        <g transform="translate(0,170)">
          <rect x="20" y="0" width="125" height="24" rx="2" fill="rgba(11,14,20,.7)" stroke="#1e2530"/>
          <text x="32" y="15" font-size="9.5" fill="#9aa4b2" font-weight="700">AGREEMENT</text>
          ${_agreeSvg}
        </g>
        <text x="590" y="232" text-anchor="middle" font-family="sans-serif" font-size="10.5" fill="#9aa4b2">Cell color = BUY / HOLD / SELL · gray = N/A · agreement = mode-share per column.</text>
      </svg>
    </div>`;

  // === ⑤ CONVICTION CHECKLIST ===
  const _rules = [
    { name:'BAP ≥ regime floor',   pass: score >= regimeF,
      evid: `BAP ${score} vs floor ${regimeF}` },
    { name:'Entry quality OK',     pass: ['FRESH','PULLBACK','VALID'].includes(eqRaw),
      evid: `entry ${eqRaw || '—'}` },
    { name:'R:R ≥ 2.0',            pass: !isNaN(rr) && rr >= 2.0,
      evid: `R:R ${isNaN(rr)?'—':rr.toFixed(2)}` },
    { name:'Quant edge ≥ 0',       pass: !isNaN(_edgePp) && _edgePp >= 0,
      evid: `edge ${_edgeTxt}` },
    { name:'No failed hard gate',  pass: failedGates.length === 0,
      evid: failedGates.length ? `${failedGates.length} failed` : 'all clear' },
  ];
  const _passN = _rules.filter(r => r.pass).length;
  const _convChkTier = (_passN >= 5) ? 'HIGH' : (_passN >= 3) ? 'MEDIUM' : 'LOW';
  const _rulesHTML = _rules.map(r => `
    <div class="qov4-rule ${r.pass?'pass':'fail'}">
      <div class="qov4-rmark">${r.pass?'✓':'✗'}</div>
      <div>
        <div class="qov4-rname">${_escHtml(r.name)}</div>
        <div class="qov4-revid">${_escHtml(r.evid)}</div>
      </div>
      <div class="qov4-rstamp">${r.pass?'PASS':'FAIL'}</div>
    </div>`).join('');
  const _convictionHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">5</span>Conviction Checklist <span class="qov4-sub">5-rule pass · failed rules show why · pass-count → tier</span></h3>
      <div class="qov4-conv-grid">
        <div class="qov4-rules">${_rulesHTML}</div>
        <div class="qov4-tier ${_convChkTier}">
          <div class="qov4-tier-k">CONVICTION</div>
          <div class="qov4-tier-v">${_convChkTier}</div>
          <div class="qov4-tier-r">${_passN} of 5 rules pass</div>
        </div>
      </div>
    </div>`;

  // === ⑥ QUANT SIGNAL PANEL ===
  // 5 sparkline tiles — sparklines come from history if present; else flat-line at current.
  const _spark = (vals, color, w, h) => {
    if (!vals || !vals.length) return '';
    const n = vals.length;
    const lo = Math.min(...vals), hi = Math.max(...vals);
    const range = hi - lo || 1;
    return vals.map((v,i)=>`${(i/(n-1))*w},${h-((v-lo)/range)*h}`).join(' ');
  };
  // try to pull history from T
  const _pT1Hist = Array.isArray(T.p_target_history) ? T.p_target_history :
                   (Array.isArray(mc.p_target_history) ? mc.p_target_history : null);
  const _edgeHist = Array.isArray(T.edge_history) ? T.edge_history : null;
  const _regHist = Array.isArray(DATA.regime_score_history) ? DATA.regime_score_history : null;
  const _kHist = Array.isArray(T.kelly_history) ? T.kelly_history : null;
  const _cvarHist = Array.isArray(T.cvar_history) ? T.cvar_history : null;
  // synthesize at-rest flat polyline from current value if no history
  const _flatSpark = (cur, w, h) => {
    if (isNaN(cur)) return '';
    const ys = Array(8).fill(h/2);
    return ys.map((y,i)=>`${(i/7)*w},${y}`).join(' ');
  };
  const _tileBase = (x, label, color, valTxt, valColor, sub, line, fill) => `
    <g transform="translate(${x},20)">
      <rect x="-6" y="-12" width="186" height="170" rx="4" fill="rgba(22,27,36,.4)" stroke="#1e2530"/>
      <text x="0" y="0" font-size="9" fill="#9aa4b2" font-weight="700" letter-spacing=".10em">${label}</text>
      ${line ? `<polyline fill="none" stroke="${color}" stroke-width="1.6" points="${line}" transform="translate(0,40)"/>` : ''}
      <text font-size="22" fill="${valColor}" font-weight="800" x="0" y="125">${valTxt}</text>
      <text font-size="8.5" fill="#9aa4b2" x="0" y="142">${_escHtml(sub)}</text>
    </g>`;
  // build each tile with real data fallback
  const _qsP_line = _pT1Hist && _pT1Hist.length ? _spark(_pT1Hist, '#3DDC97', 170, 50) : _flatSpark(pT1First, 170, 50);
  const _qsE_line = _edgeHist && _edgeHist.length ? _spark(_edgeHist, '#3DDC97', 170, 50) : _flatSpark(_edgePp, 170, 50);
  const _qsR_line = _regHist && _regHist.length ? _spark(_regHist, '#3DDC97', 170, 50) : _flatSpark(score, 170, 50);
  const _qsK_line = _kHist && _kHist.length ? _spark(_kHist, '#5EA1FF', 170, 50) : _flatSpark(finalAlloc, 170, 50);
  const _qsC_line = _cvarHist && _cvarHist.length ? _spark(_cvarHist, '#E5484D', 170, 50) : _flatSpark(cvar975, 170, 50);
  const _quantPanelHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">6</span>Quant Signal Panel <span class="qov4-sub">P(target) · Edge · Regime · Kelly · CVaR · 30d history (flat = no history)</span></h3>
      <svg viewBox="0 0 1080 200" style="width:100%;height:auto;display:block" font-family="monospace">
        ${_tileBase(20,  'P(TARGET)',    '#3DDC97', _pT1Pct, '#e6eaf2', !isNaN(setupN_hist) ? `n=${setupN_hist}${setupLB_all!=null?` · LB ${(setupLB_all*100).toFixed(0)}%`:''}` : 'no setup history', _qsP_line)}
        ${_tileBase(220, 'EDGE (signed)', '#3DDC97', _edgeTxt, _edgeColor, !isNaN(_edgePp) ? 'MC P(T1)−P(stop)' : 'no monte_carlo', _qsE_line)}
        ${_tileBase(420, 'REGIME SCORE',  '#3DDC97', isNaN(score)?'—':score.toFixed(0), '#3DDC97', `regime ${regime} · floor ${regimeF}`, _qsR_line)}
        ${_tileBase(620, 'KELLY SIZE',    '#5EA1FF', _sizeTxt, _sizeColor, _sizeSub, _qsK_line)}
        ${_tileBase(820, 'CVaR-97.5',     '#E5484D', _cvarTxt, _cvarColor, !isNaN(cvar975) ? '10d · 97.5% tail' : 'no fwd_dist', _qsC_line)}
        <text x="540" y="190" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#6b7280">historical sparklines plot when arrays present on T (p_target_history / edge_history / kelly_history / cvar_history); otherwise show flat-line stub.</text>
      </svg>
    </div>`;

  // === ⑦ PRICE CHART · TRADE PLAN ===
  const _prices60 = (Array.isArray(T.prices_60d) && T.prices_60d.length) ? T.prices_60d
                  : (Array.isArray(T.price_60d) && T.price_60d.length) ? T.price_60d
                  : (Array.isArray(T.ohlcv_60d) ? T.ohlcv_60d.map(b => b.close ?? b[4]) : null);
  let _priceChartBody;
  if (!_prices60 || !_prices60.length || _prices60.length < 10) {
    _priceChartBody = _stub('PRICE CHART', 'no prices_60d / ohlcv_60d on T');
  } else {
    const _emaArr = (vals, n) => {
      const k = 2/(n+1); const out = [];
      vals.forEach((v,i) => { out.push(i===0 ? v : v*k + out[i-1]*(1-k)); });
      return out;
    };
    const _rsiArr = (vals, n=14) => {
      if (vals.length <= n) return Array(vals.length).fill(50);
      const out = Array(vals.length).fill(50);
      let gain=0, loss=0;
    ...
    const e8 = _emaArr(_prices60, 8);
    const e20 = _emaArr(_prices60, 20);
    const e50 = _emaArr(_prices60, 50);
    const e100 = _emaArr(_prices60, 100);
    const rsiVals = _rsiArr(_prices60, 14);
    const allYs = [..._prices60, ...e8, ...e20, ...e50, ...e100, stop, t1, t2].filter(v => !isNaN(v) && v != null);
    const yLo = Math.min(...allYs)*0.98, yHi = Math.max(...allYs)*1.02;
    const yRange = yHi - yLo || 1;
    const xN = _prices60.length;
    const sx = i => 60 + (i/(xN-1))*960;
    const sy = v => 40 + (1 - (v-yLo)/yRange)*230;  // y 40..270, price band
    const _poly = arr => arr.map((v,i)=>`${sx(i)},${sy(v)}`).join(' ');
    const _yT1 = !isNaN(t1) ? sy(t1) : null;
    const _yT2 = !isNaN(t2) ? sy(t2) : null;
    const _yStop = !isNaN(stop) ? sy(stop) : null;
    const _yELo = !isNaN(eLo) ? sy(eLo) : null;
    const _yEHi = !isNaN(eHi) ? sy(eHi) : null;
    const _yNow = sy(_prices60[xN-1]);
    // RSI scale (y 370–425)
    const rsiPoly = rsiVals.map((v,i)=>`${sx(i)},${370 + (1-v/100)*55}`).join(' ');
    // EMA stack check
    const lastE8 = e8[xN-1], lastE20 = e20[xN-1], lastE50 = e50[xN-1], lastE100 = e100[xN-1];
    const stackBull = (lastE8 > lastE20 && lastE20 > lastE50 && lastE50 > lastE100);
    const stackBear = (lastE8 < lastE20 && lastE20 < lastE50 && lastE50 < lastE100);
    const stackTxt = stackBull ? '8 > 20 > 50 > 100  ✓' : stackBear ? '8 < 20 < 50 < 100  ✗' : 'mixed · no stack';
    const stackColor = stackBull ? '#3DDC97' : stackBear ? '#E5484D' : '#F5A524';
    _priceChartBody = `
      <svg viewBox="0 0 1100 510" style="width:100%;height:auto;display:block">
        <defs><linearGradient id="qov4-priceGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#5EA1FF" stop-opacity=".18"/><stop offset="1" stop-color="#5EA1FF" stop-opacity="0"/></linearGradient></defs>
        <g stroke="#1e2530" stroke-width=".5">
          <line x1="60" y1="40"  x2="1020" y2="40"/>
          <line x1="60" y1="90"  x2="1020" y2="90"/>
          <line x1="60" y1="140" x2="1020" y2="140"/>
          <line x1="60" y1="190" x2="1020" y2="190"/>
          <line x1="60" y1="240" x2="1020" y2="240"/>
        </g>
        ${_yT2!=null ? `<line x1="60" y1="${_yT2}" x2="1020" y2="${_yT2}" stroke="#3DDC97" stroke-width=".9" stroke-dasharray="6 3"/><text x="1024" y="${_yT2+3}" font-family="monospace" font-size="10.5" fill="#3DDC97" font-weight="700">T2 $${t2.toFixed(2)}</text>` : ''}
        ${_yT1!=null ? `<line x1="60" y1="${_yT1}" x2="1020" y2="${_yT1}" stroke="#3DDC97" stroke-width=".9" stroke-dasharray="6 3"/><text x="1024" y="${_yT1+3}" font-family="monospace" font-size="10.5" fill="#3DDC97" font-weight="700">T1 $${t1.toFixed(2)}</text>` : ''}
        ${_yELo!=null && _yEHi!=null ? `<rect x="60" y="${Math.min(_yELo,_yEHi)}" width="960" height="${Math.abs(_yEHi-_yELo)||5}" fill="rgba(94,161,255,.22)" stroke="#5EA1FF" stroke-width=".6"/><text x="1024" y="${Math.min(_yELo,_yEHi)+8}" font-family="monospace" font-size="10.5" fill="#5EA1FF" font-weight="700">entry $${eLo.toFixed(2)}–$${eHi.toFixed(2)}</text>` : ''}
        ${_yStop!=null ? `<line x1="60" y1="${_yStop}" x2="1020" y2="${_yStop}" stroke="#E5484D" stroke-width=".9" stroke-dasharray="6 3"/><text x="1024" y="${_yStop+3}" font-family="monospace" font-size="10.5" fill="#E5484D" font-weight="700">stop $${stop.toFixed(2)}</text>` : ''}
        <polyline points="${_poly(e100)}" fill="none" stroke="#818CF8" stroke-width="1.2" stroke-dasharray="3 2" opacity=".85"/>
        <polyline points="${_poly(e50)}"  fill="none" stroke="#C084FC" stroke-width="1.3" opacity=".9"/>
        <polyline points="${_poly(e20)}"  fill="none" stroke="#F5A524" stroke-width="1.5" opacity=".95"/>
        <polyline points="${_poly(e8)}"   fill="none" stroke="#FCD34D" stroke-width="1.6"/>
        <polyline points="${_poly(_prices60)}" fill="none" stroke="#e6eaf2" stroke-width="1.9" stroke-linejoin="round"/>
        <circle cx="${sx(xN-1)}" cy="${_yNow}" r="5" fill="#5EA1FF" stroke="#0b0e14" stroke-width="2"/>
        <line x1="${sx(xN-1)}" y1="20" x2="${sx(xN-1)}" y2="250" stroke="#5EA1FF" stroke-width=".6" stroke-dasharray="2 3"/>
        <text x="${sx(xN-1)}" y="14" text-anchor="middle" font-family="monospace" font-size="10" fill="#5EA1FF" font-weight="700">now $${_prices60[xN-1].toFixed(2)}</text>
        <g id="qov4-rsiPanel">
          <line x1="60" y1="370" x2="1020" y2="370" stroke="#1e2530" stroke-width=".6"/>
          <line x1="60" y1="425" x2="1020" y2="425" stroke="#1e2530" stroke-width=".6"/>
          <text x="50" y="380" text-anchor="end" font-family="monospace" font-size="9" fill="#9aa4b2" font-weight="700">RSI 14</text>
          <line x1="60" y1="386.5" x2="1020" y2="386.5" stroke="#E5484D" stroke-width=".6" stroke-dasharray="3 2"/>
          <text x="1024" y="389" font-family="monospace" font-size="8" fill="#E5484D" font-weight="700">70 OB</text>
          <line x1="60" y1="397.5" x2="1020" y2="397.5" stroke="#2a3340" stroke-width=".5" stroke-dasharray="2 2"/>
          <text x="1024" y="400" font-family="monospace" font-size="8" fill="#6b7280">50</text>
          <line x1="60" y1="408.5" x2="1020" y2="408.5" stroke="#3DDC97" stroke-width=".6" stroke-dasharray="3 2"/>
          <text x="1024" y="411" font-family="monospace" font-size="8" fill="#3DDC97" font-weight="700">30 OS</text>
          <polyline points="${rsiPoly}" fill="none" stroke="#FCD34D" stroke-width="1.5"/>
          <text x="1024" y="423" font-family="monospace" font-size="9" fill="#FCD34D" font-weight="700">RSI ${rsiVals[xN-1].toFixed(0)}</text>
        </g>
        <g font-family="monospace" font-size="9" fill="#6b7280">
          <text x="60" y="442" text-anchor="middle">−${xN-1}d</text>
          <text x="540" y="442" text-anchor="middle">−${Math.floor((xN-1)/2)}d</text>
          <text x="1020" y="442" text-anchor="middle">today</text>
        </g>
        <g transform="translate(70,50)">
          <rect width="220" height="46" rx="3" fill="rgba(11,14,20,.85)" stroke="#1e2530"/>
          <text x="10" y="15" font-family="monospace" font-size="8.5" fill="#9aa4b2" font-weight="700" letter-spacing=".10em">EMA STACK · CURRENT</text>
          <text x="10" y="32" font-family="monospace" font-size="11" fill="${stackColor}" font-weight="800">${stackTxt}</text>
          <text x="10" y="42" font-family="monospace" font-size="8.5" fill="#9aa4b2">${stackBull?'bullish · trend intact':stackBear?'bearish · trend down':'transition'}</text>
        </g>
        <text x="540" y="490" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#6b7280">${xN} daily closes · EMAs/RSI computed from price · targets/stop from canonical_trade_plan</text>
      </svg>`;
  }
  const _priceChartHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">7</span>Price Chart · Trade Plan <span class="qov4-sub">EMA 8/20/50/100 · stop/T1/T2/entry overlays · RSI(14)</span></h3>
      ${_priceChartBody}
    </div>`;

  // === ⑧ R:R ASYMMETRY + EV WATERFALL ===
  let _rrEvBody;
  if (isNaN(pX) || isNaN(stop) || isNaN(t1) || pX<=0 || stop<=0 || t1<=0) {
    _rrEvBody = _stub('R:R / EV WATERFALL', 'no stop / T1 / price');
  } else {
    // Map stop $ to x=60, now to scaled, T1 to x=1020
    const dStop = pX - stop;     // > 0 (long)
    const dT1   = t1 - pX;       // > 0 (long)
    const dT2   = (!isNaN(t2) && t2>0) ? (t2 - pX) : null;
    const total = dStop + dT1;
    const xNow = 60 + (dStop/total)*960;
    // probabilities — use monte_carlo if present
    const pBest = !isNaN(mc.p_hit_t2) ? mc.p_hit_t2 : (!isNaN(pT1First) ? Math.max(0, pT1First*0.3) : 0.18);
    const pBase = !isNaN(pT1First) ? Math.max(0, pT1First - pBest) : 0.45;
    const pWorst = !isNaN(pStopFirst) ? pStopFirst : 0.30;
    const evBest = pBest * (dT2 != null ? dT2 : dT1*1.5);
    const evBase = pBase * dT1;
    const evWorst = pWorst * -dStop;
    const evTotal = evBest + evBase + evWorst;
    const evColor = evTotal >= 0 ? '#3DDC97' : '#E5484D';
    const evTotalPct = pX > 0 ? (evTotal/pX*100) : 0;
    _rrEvBody = `
      <svg viewBox="0 0 1080 150" style="width:100%;height:auto;display:block">
        <rect x="60" y="50" width="${xNow-60}" height="40" fill="#E5484D" opacity=".85" rx="2"/>
        <rect x="${xNow}" y="50" width="${1020-xNow}" height="40" fill="#3DDC97" opacity=".85" rx="2"/>
        <line x1="60" y1="45" x2="60" y2="100" stroke="#E5484D" stroke-width="1.5"/>
        <text x="60" y="36" font-family="monospace" font-size="11" fill="#E5484D" font-weight="700" text-anchor="middle">STOP $${stop.toFixed(2)}</text>
        <text x="60" y="120" font-family="monospace" font-size="10" fill="#E5484D" text-anchor="middle">${stopPct.toFixed(1)}% (−$${dStop.toFixed(2)})</text>
        <line x1="${xNow}" y1="40" x2="${xNow}" y2="100" stroke="#5EA1FF" stroke-width="2"/>
        <circle cx="${xNow}" cy="70" r="6" fill="#5EA1FF" stroke="#0b0e14" stroke-width="2"/>
        <text x="${xNow}" y="32" font-family="monospace" font-size="11" fill="#5EA1FF" font-weight="700" text-anchor="middle">NOW $${pX.toFixed(2)}</text>
        <line x1="1020" y1="45" x2="1020" y2="100" stroke="#3DDC97" stroke-width="1.5"/>
        <text x="1020" y="36" font-family="monospace" font-size="11" fill="#3DDC97" font-weight="700" text-anchor="middle">T1 $${t1.toFixed(2)}</text>
        <text x="1020" y="120" font-family="monospace" font-size="10" fill="#3DDC97" text-anchor="middle">+${t1Pct.toFixed(1)}% (+$${dT1.toFixed(2)})</text>
        <g transform="translate(540,68)" font-family="monospace">
          <text font-size="11" fill="#9aa4b2" text-anchor="middle" y="-12">R-ratio (current → T1)</text>
          <text font-size="22" fill="${_rrColor}" font-weight="800" text-anchor="middle" y="12">${_rrTxt}</text>
        </g>
      </svg>
      <h3 style="margin-top:16px"><span class="qov4-num">8b</span>Expected Value Waterfall <span class="qov4-sub">scenario probability × outcome → EV contribution</span></h3>
      <svg viewBox="0 0 1100 260" style="width:100%;height:auto;display:block" font-family="monospace">
        <g font-size="9" fill="#9aa4b2" font-weight="700" letter-spacing=".10em" transform="translate(0,18)">
          <text x="20">SCENARIO</text><text x="200">OUTCOME</text><text x="350">PROB</text>
          <text x="430">$ MOVE</text><text x="540">PROB BAR</text><text x="830">EV $ CONTRIBUTION</text>
        </g>
        <line x1="20" y1="26" x2="1080" y2="26" stroke="#1e2530" stroke-width=".6"/>
        <g transform="translate(0,40)">
          <rect x="20" y="0" width="1060" height="40" rx="3" fill="rgba(61,220,151,.05)"/>
          <text x="32" y="16" font-size="12" fill="#3DDC97" font-weight="700">▲ BEST</text>
          <text x="32" y="32" font-size="9.5" fill="#9aa4b2">${dT2 != null ? 'T2 reached' : 'T1 + extension'}</text>
          <text x="200" y="24" font-size="13" fill="#3DDC97" font-weight="700">$${(dT2 != null ? t2 : (pX+dT1*1.5)).toFixed(2)}</text>
          <text x="350" y="24" font-size="13" fill="#e6eaf2" font-weight="800">${(pBest*100).toFixed(0)}%</text>
          <text x="430" y="24" font-size="13" fill="#3DDC97" font-weight="700">+$${(dT2 != null ? dT2 : dT1*1.5).toFixed(2)}</text>
          <rect x="540" y="14" width="280" height="14" fill="rgba(107,114,128,.10)" rx="2"/>
          <rect x="540" y="14" width="${pBest*280}" height="14" fill="#3DDC97" rx="2"/>
          <text x="830" y="24" font-size="13" fill="#3DDC97" font-weight="800">${evBest>=0?'+':''}$${evBest.toFixed(2)}</text>
        </g>
        <g transform="translate(0,90)">
          <rect x="20" y="0" width="1060" height="40" rx="3" fill="rgba(94,161,255,.05)"/>
          <text x="32" y="16" font-size="12" fill="#5EA1FF" font-weight="700">► BASE</text>
          <text x="32" y="32" font-size="9.5" fill="#9aa4b2">T1 reached</text>
          <text x="200" y="24" font-size="13" fill="#5EA1FF" font-weight="700">$${t1.toFixed(2)}</text>
          <text x="350" y="24" font-size="13" fill="#e6eaf2" font-weight="800">${(pBase*100).toFixed(0)}%</text>
          <text x="430" y="24" font-size="13" fill="#3DDC97" font-weight="700">+$${dT1.toFixed(2)}</text>
          <rect x="540" y="14" width="280" height="14" fill="rgba(107,114,128,.10)" rx="2"/>
          <rect x="540" y="14" width="${pBase*280}" height="14" fill="#5EA1FF" rx="2"/>
          <text x="830" y="24" font-size="13" fill="#3DDC97" font-weight="800">${evBase>=0?'+':''}$${evBase.toFixed(2)}</text>
        </g>
        <g transform="translate(0,140)">
          <rect x="20" y="0" width="1060" height="40" rx="3" fill="rgba(229,72,77,.05)"/>
          <text x="32" y="16" font-size="12" fill="#E5484D" font-weight="700">▼ WORST</text>
          <text x="32" y="32" font-size="9.5" fill="#9aa4b2">stop hit</text>
          <text x="200" y="24" font-size="13" fill="#E5484D" font-weight="700">$${stop.toFixed(2)}</text>
          <text x="350" y="24" font-size="13" fill="#e6eaf2" font-weight="800">${(pWorst*100).toFixed(0)}%</text>
          <text x="430" y="24" font-size="13" fill="#E5484D" font-weight="700">−$${dStop.toFixed(2)}</text>
          <rect x="540" y="14" width="280" height="14" fill="rgba(107,114,128,.10)" rx="2"/>
          <rect x="540" y="14" width="${pWorst*280}" height="14" fill="#E5484D" rx="2"/>
          <text x="830" y="24" font-size="13" fill="#E5484D" font-weight="800">$${evWorst.toFixed(2)}</text>
        </g>
        <line x1="20" y1="195" x2="1080" y2="195" stroke="#1e2530" stroke-width=".8"/>
        <g transform="translate(0,210)">
          <text x="32" y="14" font-size="11" fill="#9aa4b2" font-weight="700" letter-spacing=".10em">EXPECTED VALUE (per share)</text>
          <text x="32" y="30" font-size="9" fill="#6b7280">sum of probability-weighted outcomes · the true ‟return‟ of the trade</text>
          <text x="830" y="22" font-size="22" fill="${evColor}" font-weight="900">${evTotal>=0?'+':''}$${evTotal.toFixed(2)}</text>
          <text x="830" y="40" font-size="11" fill="${evColor}" font-weight="700">${evTotalPct>=0?'+':''}${evTotalPct.toFixed(2)}% on $${pX.toFixed(2)} · ${evTotal>=0?'positive EV → take the trade':'negative EV → skip'}</text>
        </g>
      </svg>`;
  }
  const _rrEvHTML = `
    <div class="qov4-panel">
      <h3><span class="qov4-num">8</span>R:R Asymmetry + EV Waterfall <span class="qov4-sub">invalidation → target bar · 3-scenario waterfall · TOTAL EV</span></h3>
      ${_rrEvBody}
    </div>`;
