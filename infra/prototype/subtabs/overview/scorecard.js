...
export function render() {
  ...
  const pillars = [
    ...
    { name:'Fundamentals', score:T.fund_score, max:T.fund_max || 30,
      summary:`Quality gate score ${T.fund_score || 0}/${T.fund_max || 30}. ${T.earn_days != null && T.earn_days <= 14 ? `Earnings binary in ${T.earn_days}d.` : 'No imminent earnings.'}`,
      tier: tier(T.fund_score || 0, T.fund_max || 30) },
    ...
    { name:'Smart Money', score:T.smc_pts ?? T.smc_score, max:T.smc_max || 10,
      summary:`SMC zones ${T.smc_pts ?? T.smc_score ?? 0}/${T.smc_max || 10}. ${T.above_200sma ? 'Above 200 SMA — Stage 2.' : 'Below 200 SMA.'}`,
      tier: tier(T.smc_pts ?? T.smc_score ?? 0, T.smc_max || 10) },
    { name:'Quality Gate', score: T.score >= 70 ? 4 : T.score >= 50 ? 3 : 2, max:5,
      summary:`${T.score >= 70 ? '4/5 hard rules clear' : T.score >= 50 ? '3/5 rules — partial pass' : '2/5 rules — gated'}. Liquidity OK, RR ${((T?.canonical_trade_plan?.risk?.rr_ratio)||T.rr||0).toFixed(1)}.`,
